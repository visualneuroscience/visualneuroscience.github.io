...
fMRIprep is a helpful and stardardized preprocessing pipeline for BIDS-valid datsets. By combining various neuroimaging software packages (FSL, ANTs, FreeSurfer,...) it allows us to run various preprocessing steps automatically:
...
To start fMRIprep, click on the bird symbol at the bottom left on neurodesk and navigate to `Neurodesk > All Applications`, and find `fmriprep`. Select the version that you want to use ![Starting fMRIprep](./images/04_fmriprep/00_OpenFmriprep.png)
...
3.  [ROI analysis](roi_analysis.qmd)
