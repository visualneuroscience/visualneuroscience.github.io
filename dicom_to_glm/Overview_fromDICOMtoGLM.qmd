...
1)  Regular scanning time:
    ...
    -   a few days later via `mri_dropoff` in the `J` drive ([Subpage for accessing other networks](../neurodesk/neurodesktop.qmd#sec-mountNetwork))
...
1)  Newerst approach (recommended): use the BIDScoin workfolw
    ...
    # sort dicoms (otherwise not all the data will be discoverable)
    dcm2niix -o [YOUR_SORTED_DICOM_DIR] -r y [YOUR_ORIGINAL_DICOM_DIR]

    # convert to bids
    # edit following the instructions on
    # https://visualneuroscience.github.io/dicom_to_glm/DICOMtoBIDS.html#sec-bidscoiner
...
2)  Older approach: using a script from Adam that can be [found here](https://github.com/adam-coates/dicom2bids) (or here: `/shared/website/dcm2bids_adam.sh`). All steps are also described in a [separate page](DICOMtoBIDS.qmd#sec-script)

3)  Older bidscoin approach: use the [BIDScoin workflow](https://bidscoin.readthedocs.io/en/latest/workflow.html) implemented in `/shared/website/dcm2bids_coiner.sh`. All steps are also described in a [separate page](DICOMtoBIDS.qmd#sec-bidscoiner)
