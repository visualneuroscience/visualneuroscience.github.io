...
-   Check if all necessary resources are sourced (e.g. did you include `ml freesurfer` before [opening a volume to check for noise scans](RemoveNoiseScan.qmd#sec-OpenFreeview))
...
-   If the content of the fmap's .json file uses a different naming convention than necessary for your version of [fMRIprep](run_fmriprep.qmd), no distortion correction is applied
...
One potential way to fix this issue is to perform manual coregistration *before* running [fMRIPrep](run_fmriprep.qmd) again. The necessary steps using `mri_coreg` or `bb_register` can be found in the following notebook: `/shared/website/pre-fmriprep.ipynb`
...
>>>>>>> 9c15005b87e84daa17b3768e2bab3c08cdd82d66
