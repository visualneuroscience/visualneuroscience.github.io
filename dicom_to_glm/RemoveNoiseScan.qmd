...
Thus, before running our [fmriprep](run_fmriprep.qmd) pipeline, we have to remove this information from our scans!
...
Navigate to your `func` directory in your [BIDS](DICOMtoBIDS.qmd#sec-bidsdirectory) directory, select one of the `.nifti` files and press `Open` afterwards ![Select functional nifti file of a run in your bids directory](./images/02_RemoveDenoising/04_Navigate_to_functional_file.png)

Now, with the field `Select volume file` containing the correct path(s), you can click `Open`![Open Volume](./images/02_RemoveDenoising/05_confirm_Selection.png)

When the volume loaded, it should look similar to this: ![First Frame of Functional Scan](./images/02_RemoveDenoising/06_0_firstFrame.png)

## Check for noise Scan {#sec-CheckDenoising}

To check if a noise scan is included in you data, use the slider next to `Frames` (left side, roughly in the middle), to navigate *to the last frame*. If a denoising sequence was used during data acquisition in the scanner, this frame looks "empty". Toggle between the last and second to last frame, to visualize the difference ![Denoising Included in your Scan](./images/02_RemoveDenoising/06_1_DenoisingScan_Confirmed.png)
...
You confirmed noise information being present in your data in the last section. Before running [fmriprep](run_fmriprep.qmd), we have to remove this information. To achieve this:
