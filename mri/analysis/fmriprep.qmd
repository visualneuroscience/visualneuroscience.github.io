...
<span style="font-variant:small-caps;"><font size="-1">NOTE: using `fitlins` does not currently work using the flag `-u $(id -u):$(id -g) the user must specify their user id directly e.g. -u 1087:1092` see post about fitlins</font></span>
...
1. I ran fMRIprep but no distortion correction happened
   ...
   - If your fmap .json file is using the BIDS uri specification in the `IntendedFor` field and you are using an older fMRIprep version then this will need to changed the older path specification e.g.
