---
title: "MRI lab setup (2024)"
aliases:
  - "/mri/mri-lab-setup-2024/index.html"
description: "How to set up and run experiments in the MRI lab: peripherals, displays, KVM switch, trigger box, PsychoPy and Psychtoolbox"
image: images/trigger-box-front.png
author: "AC"
categories: [MRI, NNL-Goggles]
date: 2024-10-07
code-links:
  - text: "response_box_check.m"
    icon: download
    href: response_box_check.m
---
...
![Goggles HDMI inputs](./images/goggle-box.png){width="75%"}
...
::: {layout-ncol=2}
![Goggles plug unplugged](./images/goggle-unplug.jpg)

![Goggles plug plugged in](./images/goggle-plug.jpg)
:::
...
![The light on the eyetracker should be illuminated red](./images/eyetracker-box.jpg){width="30%"}
...
![PopOS example settings window, always make sure to close this window and not apply new display settings](./images/pop-os-example.png)
...
![KVM switcher, button 1 = eyetracker pc, 2 = stimulus pc](./images/kvm-switch.png)
...
-   If the trigger box is not powered, plug it in
    ...
    ![](images/trigger-box-power-supply.png){width="75%"}
...
![Back of the trigger box](./images/trigger-box-back.png){width="75%"}
...
![Trigger box front](./images/trigger-box-front.png)
...
-   On the right, the top light is constantly turned on if it is connected to the computer. On the bottom, the light is blinking whenever the scanner trigger is being sent
    ...
    ![](images/trigger-box-settings.jpg){width="75%"}

    If not, press the round button and select the following settings:

-   Manual config

    ![](images/trigger-box-manual-config.jpg){width="75%"}

-   HHSC-2x4-L

    ![](images/trigger-box-hhsc-2x4-l.jpg){width="75%"}

-   USB

    ![](images/trigger-box-usb.jpg){width="75%"}

-   HID KEY 12345

    ![](images/trigger-box-hid-key-12345.jpg){width="75%"}
...
Differently from PsychoPy, PsychToolbox under Linux handles multiple keyboards independently. It is therefore important that your PsychToolbox script reads scanner trigger (Key '5%') and button box input from the correct keyboard (Trigger Box). To check the Keyboard ID of the Trigger Box and its name, run "[response_box_check.m](response_box_check.m){download="response_box_check.m"}" prior to putting the subject into the scanner. Currently, the keyboard name is 'Current Designs, Inc. 932' , but the Keyboard ID can vary depending on other devices that are plugged in.
