...
- Pasting graphs into the text
  ...
  - keep the original for high-resolution export
...
#️⃣ Word count for the introduction is typically 500-1500 words (JN is special as it sets 500 as a maximum).
...
#️⃣ Word count for the discussion: Keep the first version at roughly 1500 words. Note that the text will become longer with time due to reviewer's comments, so it is o.k. to keep the first version concise.
...
- Each paragraph has an introductory sentence, some statements inside, and a concluding sentence (see example below).

  - *Our conscious experience of the visual input is rich and complex. Viewing even a simple object, such as an apple, evokes a myriad of impressions: outline, color, texture, and 3D shape. However, common research practices tend to reduce the complexity of subjective experience to predefined categories. For example, in a near-threshold detection task, observers are asked whether they perceived a stimulus presented briefly, with a yes/no response. Similarly, when presenting an ambiguous stimulus such as the Necker cube or the Rubin’s vase illusion, the task is to report perception by choosing one of the two possible interpretations. Such simplifications are hardly compatible with complex and dynamic nature of our perception. (Yilmaz et al., 2025)*
...
- [ ] Use Styles of your word processor, especially for headings
...
# Manuscript life cycle (first author perspective)

- Create the initial draft which you and your PI are happy with. Decide on a target journal and the list of potential reviewers.

  - At this stage the manuscript undergoes several rounds of going back and froth between you and your PI. This is normal.

- Once both of you are happy with the initial draft, send this draft to the other co-authors and wait for their feedback.

- Create the final version of the manuscript for submission which incorporates co-authors feedback (potentially also discussing with the PI).

- Submit the manuscript. Important: make note of the reviewers you indicated during the submission.

- If agreed with the PI, publish the submitted version on bioRxives or similar preprint server. Send the preprint link to the co-authors. If not publishing a preprint, send a submitted copy to the co-authors for the record, notifying them about the submission.

- Once you receive the reviews, forward them to the co-authors (even if they received a copy).

- Work on the revision with the PI.

- Once both of you are happy with the revised manuscript, supplementary material and the Response letter, send all documents to the co-authors for reviewing.

- Once you heard back from the co-authors, incorporate their changes (potentially also discussing with the PI) and submit the revision.
